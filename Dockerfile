...
RUN apt-get update && apt-get install -y openssh-server dialog zip unzip bzip2 curl wget net-tools vim git sudo\
...
RUN useradd -m -U -s /bin/bash -G sudo gwinn
RUN echo 'root:root' | chpasswd
RUN echo 'gwinn:gwinn' | chpasswd
